class Skill {
  ...
  constructor(main, name, icon, desc, cd, cost, keyCode) {
    ...
    if(typeof keyCode === 'number') {
      ...
      throw new Error(`Skill ${name} can't bind the key ${keyCode}`);
    }
  ...
  bindKey() {
    window.addEventListener('keydown', (event) => {
      if ((typeof this.keyCode === 'number' && event.keyCode === this.keyCode)
        ...
        try {
          ...
          // TODO 使用更好的方式提示
          console.log('Failed to use the skill：', e.message);
        }
      }
    });
    ...
    console.log(`cxk has loaded the skill：${keyName}-${this.name}`);
  }
  ...
  cast() {
    if (this.lastCastTime + this.cd * 1000 > Date.now()) {
      throw new Error('Skill hasn not cooled down yet');
    ...
    // TODO 显示释放技能的特效
    console.log(`cxk just spent ${this.cost}Points to use the skill--${this.name}！\n${this.desc}`)
  }
}
...
class SkillQ extends Skill {
  constructor(main) {
    super(main,
      'Mind contols the ball',
      '',
      'cxk uses his mind to control the ball to turn around to hit the closest brick',
      10,
      1000,
      'Q');
  }
...
class SkillW extends Skill {
  constructor(main) {
    super(main,
      'CXK Ghost Pace',
      '',
      'cxk uses the skill he learnt in a U.S. Campus Basketball Team to make the basketball not hit the floor for 5 seconds',
      10,
      1000,
      'W');
